...
def process_item(args, tokenizer, model, item):
    ...
    scores_, probs_ = score_for_query(args, tokenizer, model, query, knowledges, cands)  # (B,N), (B,N)

    # ---- aggregate across blocks into ONE distribution over the choices ----
    if args.aggfunc == 'best_score':
        scores = torch.mean(scores_, dim=0)                 # (N,)
        probs  = torch.softmax(scores, dim=0)               # normalize across choices
    elif args.aggfunc == 'best_prob':
        probs  = torch.mean(probs_, dim=0)                  # (N,)
        probs  = probs / probs.sum()                        # ensure it sums to 1
        scores = torch.mean(scores_, dim=0)                 # optional for logging
    elif args.aggfunc == 'poe':  # product-of-experts over blocks
        logp   = torch.sum(torch.log(probs_ + 1e-12), dim=0)  # (N,)
        probs  = torch.softmax(logp, dim=0)
        scores = torch.mean(scores_, dim=0)
    elif args.aggfunc == 'moe':  # mixture-of-experts over blocks
    ...
    # ----- NEW: store full ranking and top-5 predictions -----
    # sorted indices by probability (desc)
    sorted_idx = torch.argsort(probs, descending=True)
    topk = min(5, sorted_idx.numel())
    top_idx = sorted_idx[:topk].tolist()
    top_choices = [cands[i] for i in top_idx]
    top_confs   = [float(probs[i].item()) for i in top_idx]

    # keep your existing pred fields
    p_idx = top_idx[0]
    pred  = cands[p_idx]
    pred_conf = top_confs[0]

    # also store rank-ordered info
    item['ranked_idx'] = [int(i) for i in top_idx]
    item['ranked'] = [
        {'choice': choice, 'prob': conf}
        for choice, conf in zip(top_choices, top_confs)
    ]

    # explicit pred2..pred5 (when available)
    if topk >= 2:
        item['pred2'] = top_choices[1]; item['pred2_conf'] = top_confs[1]
    if topk >= 3:
        item['pred3'] = top_choices[2]; item['pred3_conf'] = top_confs[2]
    if topk >= 4:
        item['pred4'] = top_choices[3]; item['pred4_conf'] = top_confs[3]
    if topk >= 5:
        item['pred5'] = top_choices[4]; item['pred5_conf'] = top_confs[4]

    # (unchanged) gold probability, margins, entropy, etc.
    true_prob = None
    if 'answer' in item:
        gold_txt = item['answer'].strip().lower()
        gold_idx = next((i for i, t in enumerate(cands) if t.strip().lower() == gold_txt), -1)
        if gold_idx >= 0:
            true_prob = float(probs[gold_idx].item())

    top2 = torch.topk(probs, k=min(2, probs.numel()))
    margin  = float(top2.values[0] - (top2.values[1] if top2.indices.numel() > 1 else 0.0))
    entropy = float(-(probs * torch.log(probs + 1e-12)).sum().item())

    item['probs_']    = probs_.tolist()
    item['probs']     = probs.tolist()
    item['pred']      = pred
    item['pred_conf'] = pred_conf
    if true_prob is not None:
        item['true_conf'] = true_prob
    item['conf_margin']  = margin
    item['conf_entropy'] = entropy

    if 'answer' in item:
        item['ok'] = 1 if pred == item['answer'] else 0
